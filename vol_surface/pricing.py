...
def cos_pricer(cf, S, K, T, r, option_type="call", N=128, L=12, sigma_est=None):
    """
    ...
    L : float
        Truncation range multiplier (in standard deviations)
    sigma_est : float, optional
        Estimated volatility for truncation range.
        If None, estimated from CF second moment.
    ...
    # Estimate variance for truncation range
    # For lognormal CF with parameter sigma, the variance of log(S_T) is sigma^2*T
    # Truncation range = [center +/- L * std_dev] where std_dev = sigma*sqrt(T)
    # If sigma_est is not provided, try to estimate it
    ...
    # Truncation using L * standard_deviation
    # This gives the range [log_forward - L*sigma*sqrt(T), log_forward + L*sigma*sqrt(T)]
    std_dev = sigma_est * np.sqrt(T)
