...
import numpy as np
from .iv_solver import black_scholes_call
...
def check_put_call_parity(call_price, put_price, S, K, T, r, tol=1e-3):
    ...
    return {"is_violated": diff > tol, "lhs": lhs, "rhs": rhs, "diff": diff}
...
def check_all_arbitrage(market_data, S, r, tol=1e-3):
    ...
    """
    violations = {"put_call_parity": [], "butterfly": [], "calendar": []}
        ...
        if "call_price" in row and "put_price" in row:
            ...
            if result["is_violated"]:
                violations["put_call_parity"].append(
                    {"strike": row["strike"], "expiry": row["expiry"], **result}
                )

    # Check butterfly per expiry
    for T in market_data["expiry"].unique():
        slice_data = market_data[np.abs(market_data["expiry"] - T) < 1e-6].sort_values(
            "strike"
        )
